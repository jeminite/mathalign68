...
data/
  ...
  content.json         REVIEWED TRANSCRIPTIONS — stems, choices, figures. Hand-owned: written only
                       by tools/merge_content.py from a reviewed spec; cannot be regenerated
  glyphs.json          Glyph shape clusters and their labels (many labelled by hand), read by
                       tools/extract_items.py to decode mathematics drawn as vector artwork
  alignment_citations.json  GENERATED by tools/build_citation_index.py — what a clone without
                       the guides checks alignment.json's citations against
  im_ms_reference.json IM 6-8 units, sections, lessons; standard-to-lesson table   [Phase 3]
  im_ms_pacing.json    GENERATED by tools/extract_pacing.py — day ranges, optional lessons
  im_ms_lessons_detail.json  NOT COMMITTED (gitignored): reproduces Imagine Learning's Student
                       Task Statements verbatim — see provenance/imagine_guides.md
...
**Every file in `data/` is either script-owned or hand-owned, never both, and the split is the point.**
...
is human judgement and no script ever writes it. `content.json` is the third kind: reviewed
transcription, which is judgement too and so cannot be regenerated — and, with `glyphs.json`,
it is the part of `data/` with no regenerability check behind it (`REVIEW.md` section 6). RegentsAlign mixes both in one hand-edited
...
`python3 tools/preflight.py` runs 128 more against the built site. Two of them are worth knowing
...
their exact PDF page; the site built and passing a 128-check deploy gate.
